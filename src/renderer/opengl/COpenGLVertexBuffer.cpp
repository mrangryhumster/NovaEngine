...
void COpenGLVertexBuffer::lock()
{
    ...
    if(OpenGL_VBO_Status == EVBT_VBO_STREAM or OpenGL_VBO_Status == EVBT_VBO_STATIC)
    {
        ...
        if(OpenGL_VBO_indices_ID)
        {
            ...
            u16* restored_indices = new u16[BufferSize / sizeof(u16)];
            glGetBufferSubData(GL_ELEMENT_ARRAY_BUFFER,0,BufferSize,restored_indices);
            Indices.clear();
            setIndices(restored_indices,BufferSize / sizeof(u16));
...
void COpenGLVertexBuffer::build_vbo()
{
    ...
    {
        glBindBuffer(GL_ELEMENT_ARRAY_BUFFER,OpenGL_VBO_indices_ID);
        glBufferData ( GL_ELEMENT_ARRAY_BUFFER, sizeof(u16) * IndexCount, Indices.data(), (BufferType == EVBT_VBO_STREAM) ? GL_DYNAMIC_DRAW : GL_STATIC_DRAW );
        glBindBuffer(GL_ELEMENT_ARRAY_BUFFER,0);
    }
