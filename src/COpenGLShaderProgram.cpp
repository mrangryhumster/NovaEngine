	...
	namespace renderer
	{
		...
		void COpenGLShaderProgram::setUniform(s32 uniform_location, u32 uniform_type, u32 uniform_count, const void * data)
		{
			...
			case ESUT_FLOAT_MAT4:
				glUniformMatrix4fv(uniform_location, uniform_count, GL_FALSE, static_cast<const GLfloat*>(data));
				break;
			}
		}
		//-------------------------------------------------------------------------------------------
		...
		//-------------------------------------------------------------------------------------------
		void COpenGLShaderProgram::clearConstantLocations()
		{
			m_ProgramConstant_location_mvpmatrix = -1;
			m_ProgramConstant_location_projectionmatrix = -1;
			m_ProgramConstant_location_viewmatrix = -1;
			m_ProgramConstant_location_modelmatrix = -1;
		}
		//-------------------------------------------------------------------------------------------
		void COpenGLShaderProgram::setConstant(u32 constant_type, const void* constant_data)
		{
			switch (constant_type)
			{
			case EOGL_PC_MVPMATRIX:
				if (m_ProgramConstant_location_mvpmatrix >= 0)
					glUniformMatrix4fv(m_ProgramConstant_location_mvpmatrix, 1, GL_FALSE, static_cast<const GLfloat*>(constant_data));
				break;
			case EOGL_PC_PROJECTIONMATRIX:
				if (m_ProgramConstant_location_projectionmatrix >= 0)
					glUniformMatrix4fv(m_ProgramConstant_location_projectionmatrix, 1, GL_FALSE, static_cast<const GLfloat*>(constant_data));
				break;
			case EOGL_PC_VIEWMATRIX:
				if (m_ProgramConstant_location_viewmatrix >= 0)
					glUniformMatrix4fv(m_ProgramConstant_location_viewmatrix, 1, GL_FALSE, static_cast<const GLfloat*>(constant_data));
				break;
			case EOGL_PC_MODELMATRIX:
				if (m_ProgramConstant_location_modelmatrix >= 0)
					glUniformMatrix4fv(m_ProgramConstant_location_modelmatrix, 1, GL_FALSE, static_cast<const GLfloat*>(constant_data));
				break;
			default:
				break;
			}
		...
		void COpenGLShaderProgram::update_uniform_info()
		{
			...
			if (UniformCount > 0)
			{
				...
				for (int index = 0; index < UniformCount; index++)
				{
					...
					if (UniformName == "ne_mvpmatrix" && EngineUniformType == ESUT_FLOAT_MAT4 && UniformSize == 1)
						m_ProgramConstant_location_mvpmatrix = UniformLocation;
					else if (UniformName == "ne_projectionmatrix" && EngineUniformType == ESUT_FLOAT_MAT4 && UniformSize == 1)
						m_ProgramConstant_location_projectionmatrix = UniformLocation;
					else if (UniformName == "ne_viewmatrix" && EngineUniformType == ESUT_FLOAT_MAT4 && UniformSize == 1)
						m_ProgramConstant_location_viewmatrix = UniformLocation;
					else if (UniformName == "ne_modelmatrix" && EngineUniformType == ESUT_FLOAT_MAT4 && UniformSize == 1)
						m_ProgramConstant_location_modelmatrix = UniformLocation;
					//-----------------------			
